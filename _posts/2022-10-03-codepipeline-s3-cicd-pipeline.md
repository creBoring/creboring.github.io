...
Amazon S3는 Object형 저장소를 제공하는 아마존의 대표 서비스입니다. 혹시 여러분은 S3의 정적 웹호스팅 기능도 사용해보셨나요?

만약 현재 프로젝트에서 웹서버가 **단순 정적 파일들을 제공하는 용도로만 사용되고 있거나**, **마케팅용 또는 간단한 회사 소개 홈페이지를 호스팅**하고자 하신다면 S3를 이용해보시는 것을 권해드립니다.
설정이 단순할뿐만 아니라, AWS에서 제공하는 관리형 서비스이기 때문에 서버에 대한 관리포인트가 없어지고, 가격도 저렴합니다. <br>~~우리는 이것을 서버리스라고 부르기로 했어요~~

현재 게시글에서는 S3를 웹서버 또는 단순한 홈페이지로 운영하고 있는 경우, 소스코드를 S3로 배포하는 과정을 자동화 하는 방법을 다룹니다.
