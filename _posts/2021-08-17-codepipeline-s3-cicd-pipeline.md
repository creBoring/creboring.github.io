...
하나의 서비스가 **통합, 개발되어(CI) 테스트되고 운영환경에 배포되기(CD)** 까지, 일련의 과정을 단계별로 정리하고 한 단계씩 순차적으로 실행하는 시스템을 아울러 CI/CD 파이프라인이라 부릅니다.
- `CI(Continuous Integration)`: 여러 개발자들이 함께 협업을 하며 서비스를 개발할 때, **이를 지속적으로 통합하고 중재하여 꾸준히 품질을 유지하려는 DevOps 관행을 의미합니다**. 반례로는 뒤늦게 모든 개발자들의 변경사항을 한번에 통합하는 경우가 있을 수 있습니다.
- `CD(Continuous Deploy/Delivery)`: CI가 완료된 프로그램을 매끄럽게 운영서버까지 배포/전달 하는 과정을 자동화 합니다.

<figure>
    <img src="/assets/img/posts/2021-08-17/pipeline.jpg" class="img-fluid">
    <figcaption>[<a href="http://www.freepik.com">Designed by pch.vector / Freepik</a>]</figcaption>
</figure>
...
일반적으로 파이프라인들은 아래와 같은 절차가 존재합니다.
- Source
- Build
- Deploy
- etc.. (수동 승인, 테스트, ...)

각 단계마다 수행하는 작업들은 서로 다르며, **앞선 작업의 수행이 완료되어야 후행되는 작업이 수행되는 구조**를 가지고 있습니다. 보통 이런 모습을`컨베이어 벨트`에 비유하기도 합니다.
...
이러한 일련의 과정을 정리하고 코드화하여 자동화 서비스에 넘겨주게 되면, 파이프라인은 전달받은 프로세스를 하나씩 처리하며 관리자 대신 배포 작업을 수행해줄 것입니다.
